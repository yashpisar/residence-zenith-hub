...
function SuperAdminDashboard() {
  ...
  const filteredSocieties = availableSocieties.filter(
    (s) =>
  ...
  const handleDelete = (id, name) => {
    if (
      confirm(`Are you sure you want to completely delete ${name}? This action cannot be undone.`)
    ) {
      deleteSociety(id);
              ...
              <tbody className="divide-y divide-border">
                {filteredSocieties.map((society) => (
                  <tr key={society.id} className="hover:bg-muted/30">
                    ...
                    <td className="px-6 py-4">
                      <span
                        className={`px-2.5 py-1 rounded-full text-[10px] font-bold uppercase tracking-wider border ${getStatusColor(society.status)}`}
                      >
                        {society.status}
                      </span>
                    </td>
