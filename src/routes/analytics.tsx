...
const spark = (s: number) => Array.from({ length: 8 }, (_, i) => ({ v: 18 + Math.sin(i + s) * 6 + i * 2 }));
  ...
  return (
    <>
      ...
      <div className="grid gap-5 sm:grid-cols-2 xl:grid-cols-4">
        <StatCard label="Tickets closed" value={318} change={14} icon={ClipboardList} trend={spark(1)} index={0} />
        <StatCard label="Collection rate" value={92} suffix="%" change={4} icon={Banknote} tone="success" trend={spark(2)} index={1} />
        <StatCard label="Visitors logged" value={4820} change={9} icon={DoorOpen} tone="info" trend={spark(3)} index={2} />
        <StatCard label="Active residents" value={1042} change={2} icon={Users} tone="amber" trend={spark(4)} index={3} />
      </div>
          ...
          <div className="h-[300px]">
            <ResponsiveContainer width="100%" height="100%">
              <LineChart data={complaintTrend}>
                ...
                <Line type="monotone" dataKey="raised" stroke="var(--color-chart-1)" strokeWidth={3} dot={false} />
                <Line type="monotone" dataKey="resolved" stroke="var(--color-chart-4)" strokeWidth={3} dot={false} />
              </LineChart>
            </ResponsiveContainer>
          </div>
        </SurfaceCard>
          ...
          <div className="h-[300px]">
            <ResponsiveContainer width="100%" height="100%">
              <PieChart>
                <Tooltip {...tooltipStyle} />
                <Legend iconType="circle" wrapperStyle={{ fontSize: 12 }} />
                <Pie data={categorySplit} dataKey="value" nameKey="name" innerRadius={60} outerRadius={96} paddingAngle={3} stroke="none">
