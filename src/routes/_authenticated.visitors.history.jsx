...
import { visitors as mockVisitors } from "@/lib/mock-data";
...
function VisitorHistory() {
  ...
  useEffect(() => {
    const handleStatusChange = (updatedVisitor) => {
      setData((prev) =>
        prev.map((v) =>
          v.id === updatedVisitor.id || v.name === updatedVisitor.name
            ? { ...v, status: updatedVisitor.status, exit: updatedVisitor.exit || v.exit }
            : v,
        ),
      );
    };
