...
import { ArrowRight, DoorOpen, Package, Building2, Users, QrCode, Clock, Car } from "lucide-react";
import {
  Bar,
  BarChart,
  CartesianGrid,
...
import { activityFeed, visitorFlow, visitors } from "@/lib/mock-data";
import { chartTheme, tooltipStyle } from "@/lib/chart-theme";
import { EmergencyFAB } from "@/components/guard/EmergencyFAB";

export const Route = createFileRoute("/_authenticated/guard/dashboard")({
...
function GuardDashboard() {
  ...
  const stats = [
    { label: "Visitors Today", value: 42, change: 12, icon: Users, tone: "primary" },
    { label: "Inside Society", value: 18, change: 6, icon: DoorOpen, tone: "info" },
    { label: "Expected Visitors", value: 5, change: -2, icon: Clock, tone: "amber" },
    { label: "Deliveries Today", value: 24, change: 4, icon: Package, tone: "success" },
  ];
              ...
              >
                <div className="min-w-0 flex items-center gap-4">
                  <div className="h-10 w-10 shrink-0 rounded-full bg-muted flex items-center justify-center overflow-hidden">
                    <img
                      src={`https://ui-avatars.com/api/?name=${v.name}&background=random`}
                      alt={v.name}
