...
import { Search, MapPin, Building2, Users, ArrowRight, ShieldAlert, Wrench, ShieldCheck, Plus } from "lucide-react";
...
function SelectSocietyPage() {
  ...
  const StatusIcon = ({ status }: { status: string }) => {
    switch(status) {
      case 'Active': return <ShieldCheck className="size-4 mr-1 text-green-500" />;
      case 'Under Maintenance': return <Wrench className="size-4 mr-1 text-amber-500" />;
      default: return <ShieldAlert className="size-4 mr-1 text-red-500" />;
    }
  };
        ...
        <AnimatePresence>
          {filteredAndSortedSocieties.length > 0 ? (
            ...
            <div className="flex flex-col items-center justify-center py-20 text-center">
              ...
              <p className="text-muted-foreground max-w-sm">We couldn't find any societies matching your current filters. Try adjusting your search criteria.</p>
              <Button variant="outline" className="mt-6" onClick={() => { setSearch(""); setCityFilter("All"); setTypeFilter("All"); }}>
                Clear Filters
              </Button>
            </div>
          )}
