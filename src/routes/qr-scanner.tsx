...
function QRScanner() {
  const [scanState, setScanState] = useState<"idle" | "scanning" | "valid" | "invalid" | "expired">("idle");
  ...
  const startScan = () => {
    ...
    setTimeout(() => {
      ...
      const outcome = mockOutcomes[Math.random() > 0.3 ? 0 : Math.floor(Math.random() * mockOutcomes.length)];
            ...
            <AnimatePresence mode="wait">
              {scanState === "idle" && (
                ...
                >
                  ...
                  <p className="text-muted-foreground mb-8">Position the visitor's QR code in front of the camera to verify their pass.</p>
                  <Button size="lg" onClick={startScan} className="w-full max-w-xs rounded-full shadow-lg h-14 text-lg">
                ...
                >
                  ...
                  <div className="w-full bg-muted/30 rounded-xl p-5 mb-8 border border-border">
                    <div className="grid grid-cols-2 gap-y-4 text-sm">
                      <div>
                        <p className="text-muted-foreground uppercase text-xs font-semibold">Name</p>
                        <p className="font-bold text-lg">{visitorData.name}</p>
                      </div>
                      <div>
                        <p className="text-muted-foreground uppercase text-xs font-semibold">Flat</p>
                        <p className="font-bold text-lg">{visitorData.flat}</p>
                      </div>
                      <div>
                        <p className="text-muted-foreground uppercase text-xs font-semibold">Purpose</p>
                        <p className="font-medium">{visitorData.purpose}</p>
                      </div>
                      <div>
                        <p className="text-muted-foreground uppercase text-xs font-semibold">Valid Until</p>
                  ...
                  <div className="flex gap-4 w-full">
                    <Button variant="outline" onClick={() => setScanState("idle")} className="flex-1 h-12">Cancel</Button>
                    <Button onClick={handleCheckIn} className="flex-1 h-12 bg-success hover:bg-success/90">Allow Entry</Button>
                  </div>
                </motion.div>
              )}
              ...
              {scanState === "invalid" && (
                ...
                >
                  ...
                  <p className="text-muted-foreground mb-8 max-w-sm">This QR code is not recognized by the system or might belong to a different society.</p>
                  <Button size="lg" onClick={() => setScanState("idle")} variant="outline" className="w-full max-w-xs h-14">Scan Again</Button>
                </motion.div>
              )}
              ...
              {scanState === "expired" && (
                ...
                >
                  ...
                  <p className="text-muted-foreground mb-8 max-w-sm">This visitor pass has exceeded its expected exit time or is from a past date. A new approval is required.</p>
                  <Button size="lg" onClick={() => setScanState("idle")} variant="outline" className="w-full max-w-xs h-14">Scan Again</Button>
                </motion.div>
              )}
            </AnimatePresence>
