...
export function VisitorPass({ visitor, onClose }: VisitorPassProps) {
  const handlePrint = () => {
    window.print();
  };
  ...
  return (
    ...
    >
      ...
      <div className="p-6 flex flex-col items-center bg-card">
        ...
        <div className="h-24 w-24 rounded-full border-4 border-muted overflow-hidden bg-muted mb-4 shadow-md -mt-12 relative z-10 bg-card">
          {visitor.visitorPhoto ? (
            <img src={visitor.visitorPhoto} alt={visitor.visitorName} className="h-full w-full object-cover" />
          ) : (
        ...
        <div className="w-full grid grid-cols-2 gap-y-4 gap-x-2 mt-6 p-4 bg-muted/30 rounded-xl text-sm">
          <div>
            <p className="text-xs text-muted-foreground uppercase tracking-wider font-semibold">Flat</p>
            <p className="font-bold text-foreground">{visitor.wing} - {visitor.flatNumber}</p>
          </div>
          <div>
            <p className="text-xs text-muted-foreground uppercase tracking-wider font-semibold">Host</p>
            <p className="font-bold text-foreground truncate">{visitor.residentName || "Resident"}</p>
          </div>
          <div>
            <p className="text-xs text-muted-foreground uppercase tracking-wider font-semibold">Entry Time</p>
            <p className="font-bold text-foreground">{visitor.entryTime}</p>
          </div>
          <div>
            <p className="text-xs text-muted-foreground uppercase tracking-wider font-semibold">Valid Until</p>
      ...
      <div className="p-4 bg-muted/10 border-t border-border grid grid-cols-3 gap-2">
        <Button variant="outline" size="sm" className="flex flex-col h-auto py-2 gap-1 text-xs" onClick={handlePrint}>
          <Printer className="size-4" /> Print
        </Button>
        <Button variant="outline" size="sm" className="flex flex-col h-auto py-2 gap-1 text-xs" onClick={handleDownload}>
          <Download className="size-4" /> Download
        </Button>
        <Button variant="outline" size="sm" className="flex flex-col h-auto py-2 gap-1 text-xs" onClick={onClose}>
