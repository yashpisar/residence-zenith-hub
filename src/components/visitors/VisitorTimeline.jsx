...
export function VisitorTimeline({ events }) {
    ...
    <div className="relative pl-6 space-y-6 before:absolute before:inset-0 before:ml-8 before:-translate-x-px before:h-full before:w-0.5 before:bg-gradient-to-b before:from-border before:to-transparent">
      {events.map((event, idx) => {
        ...
        return (
          ...
          >
            <div
              className={`absolute -left-6 flex size-5 items-center justify-center rounded-full bg-background ring-4 ring-background`}
            >
              <div className={`size-3 rounded-full ${config.bg.replace("/10", "")}`} />
            </div>

            <div
              className={`flex size-10 shrink-0 items-center justify-center rounded-full ${config.bg} ${config.color} ring-4 ring-background`}
            >
            ...
            <div className="flex-1 rounded-lg border border-border bg-card p-4 shadow-sm">
              ...
              <div className="mt-1 flex items-center gap-2 text-xs text-muted-foreground">
                ...
                <span className="px-1.5 py-0.5 rounded-full bg-muted text-[10px] uppercase font-bold tracking-wider">
                  {event.role}
                </span>
              </div>
              {event.remarks && (
